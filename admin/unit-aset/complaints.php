...
if ($workflow_filter !== 'all') {
    $sql .= " AND c.workflow_status = ?";
    $params[] = $workflow_filter;
}

if (!empty($search)) {
    $sql .= " AND (c.ticket_number LIKE ? OR c.perkara LIKE ? OR c.nama_pengadu LIKE ?)";
    $search_param = "%{$search}%";
    $params[] = $search_param;
    $params[] = $search_param;
    $params[] = $search_param;
...
<head>
    ...
    <title>Senarai Aduan - Unit Aset</title>
            ...
            <form method="GET" class="flex flex-wrap gap-4">
                <div class="flex-1 min-w-[200px]">
                    <input type="text" name="search" value="<?php echo htmlspecialchars($search); ?>"
                        placeholder="Cari (No. Tiket, Perkara, Nama)"
                        class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent">
                </div>
                <div>
                    <select name="workflow_status" class="px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500">
                        <option value="all" <?php echo $workflow_filter === 'all' ? 'selected' : ''; ?>>Semua Status</option>
                        <option value="dimajukan_unit_aset" <?php echo $workflow_filter === 'dimajukan_unit_aset' ? 'selected' : ''; ?>>Baru Diterima</option>
                        <option value="dalam_semakan_unit_aset" <?php echo $workflow_filter === 'dalam_semakan_unit_aset' ? 'selected' : ''; ?>>Dalam Semakan</option>
                        <option value="dimajukan_pegawai_pelulus" <?php echo $workflow_filter === 'dimajukan_pegawai_pelulus' ? 'selected' : ''; ?>>Dimajukan ke Pelulus</option>
                        <option value="diluluskan" <?php echo $workflow_filter === 'diluluskan' ? 'selected' : ''; ?>>Diluluskan</option>
                        <option value="selesai" <?php echo $workflow_filter === 'selesai' ? 'selected' : ''; ?>>Selesai</option>
                    </select>
                </div>
                    ...
                    <tbody>
                        ...
                        <?php else: ?>
                            ...
                            <tr class="border-b border-gray-100 hover:bg-gray-50">
                                <td class="py-3 px-4">
                                    <span class="font-semibold text-blue-600"><?php echo htmlspecialchars($complaint['ticket_number']); ?></span>
                                </td>
                                ...
                                <td class="py-3 px-4 text-sm"><?php echo date('d/m/Y H:i', strtotime($complaint['updated_at'])); ?></td>
                                <td class="py-3 px-4">
                                    <a href="view_complaint.php?id=<?php echo $complaint['id']; ?>"
                                       class="text-blue-600 hover:text-blue-800 mr-3" title="Lihat & Proses">
